...
#include "OmniaCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"
#include "Math/RotationMatrix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(OmniaCameraMode_ThirdPerson)

namespace OmniaCameraMode_ThirdPerson_Statics
{
...
UOmniaCameraMode_ThirdPerson::UOmniaCameraMode_ThirdPerson()
{
	...
	PenetrationAvoidanceFeelers.Add(FOmniaPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FOmniaPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FOmniaPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FOmniaPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FOmniaPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FOmniaPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UOmniaCameraMode_ThirdPerson::OnActivation_Implementation()
{
...
void UOmniaCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UOmniaCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IOmniaCameraAssistInterface* TargetControllerAssist = Cast<IOmniaCameraAssistInterface>(TargetController);

	IOmniaCameraAssistInterface* TargetActorAssist = Cast<IOmniaCameraAssistInterface>(TargetActor);
	...
	IOmniaCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IOmniaCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		{
			for (IOmniaCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UOmniaCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	{
		FOmniaPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(OmniaCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UOmniaCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UOmniaCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
